...
func processEvent(env *C.JNIEnv, e *C.AInputEvent) {
	...
	case C.AINPUT_EVENT_TYPE_MOTION:
		// EchoVR Cosmetics patch: only touch-style actions are touches.
		//
		// Upstream turns every motion action that is not a press or release into
		// a touch move. A phone only sends those, but a Quest's controller ray
		// sends a steady stream of HOVER_* events while merely pointing at the
		// panel, and the thumbsticks send SCROLL. Read as touch moves they look
		// like a finger dragging, so the UI scrolls on its own and every tap
		// turns into a drag. Hover and scroll carry no press, so drop them.
		switch C.AMotionEvent_getAction(e) & C.AMOTION_EVENT_ACTION_MASK {
		case C.AMOTION_EVENT_ACTION_HOVER_MOVE, C.AMOTION_EVENT_ACTION_HOVER_ENTER,
			C.AMOTION_EVENT_ACTION_HOVER_EXIT, C.AMOTION_EVENT_ACTION_SCROLL:
			return
		}
