...
func (c *CPip) ToCosmeticEntry() (data.CosmeticEntry, error) {
	...
	foo.CEntry.CosmeticTypeSymbol = int64(data.ToSymbol("pip"))
...
func SetupUI(state *data.AppState) fyne.CanvasObject {
	...
	pipList = widget.NewList(
		func() int { return len(state.CategoryFiltered["Pips"]) },
		func() fyne.CanvasObject { return widget.NewLabel("Template") },
		func(id widget.ListItemID, item fyne.CanvasObject) {
			realIdx := state.CategoryFiltered["Pips"][id]
...
func LoadToEditor(state *data.AppState, realIdx int) {
	...
	replacePipBtn = widget.NewButton("Replace Pip Texture", func() {
		data.HandleTextureReplacement(state, data.SymbolToHex(t.TextureSymbol), state.CurrentReplacementPath, replacePipBtn, "Replacing Pip...")
	})
	if state.CurrentReplacementPath == "" {
		replacePipBtn.Disable()
	}
	...
	state.CategoryEditor.Objects = []fyne.CanvasObject{
		...
		widget.NewLabel("Pip Texture Replacement"),
		selectPipPngBtn,
		container.NewGridWithColumns(2,
			container.NewBorder(widget.NewLabel("Original"), nil, nil, nil, pipPreviewImage),
			container.NewBorder(widget.NewLabel("Replacement"), nil, nil, nil, pipReplacementImage),
		),
		replacePipBtn,
	}
	state.CategoryEditor.Refresh()
	data.RefreshAssetPreview(state, pipPreviewImage, pipReplacementImage, state.CurrentOriginalAssetPath, state.CurrentReplacementPath, true)
